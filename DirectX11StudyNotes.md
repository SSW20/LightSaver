...
게임 안에서의 문·계단 통과와 시각적 이음새는 계속 확인해야 한다.

## 42. SoundManager와 게임 사운드 연결

`SoundManager`는 `AudioEngine` 하나와 게임에서 재사용할 `SoundEffect`들을 보관하는 싱글톤이다. 짧은 2D 효과음은 `Play2D()`로 즉시 재생하고, 발전기와 몬스터처럼 월드 위치가 중요한 소리는 `Play3D()`로 재생한다. 3D 소리는 매 프레임 플레이어 카메라의 위치·정면·위쪽 방향을 Listener에 넣어서 좌우 음량과 거리에 따른 감쇠를 계산한다.

```text
2D: 플레이어 발소리, 호흡, 피격, 손전등, UI, 승패음, 배경음악
3D: 발전기 작동/완료, 몬스터 발소리/포효
```

반복음은 매 프레임 새로 재생하지 않는다. `StartLoop2D/3D()`가 `SoundEffectInstance` 하나를 만들고 계속 재생하며, `StopLoop()`가 그 Instance를 정지하고 제거한다. 심장음은 몬스터와의 거리가 18 안으로 들어오면 시작되고 가까워질수록 커진다. 숨소리도 피해 여부와 관계없이 같은 거리 비율을 사용하되, 여러 녹음 중 하나를 일정 간격으로 골라 재생한다. 발전기는 수리가 완료되면 완료음을 한 번 재생하면서 기계 루프를 시작하고, Reset 전까지 루프를 유지한다.

플레이어와 몬스터 발소리는 실제 이동이 성공했을 때 각자의 타이머가 0 이하인 경우에만 재생한다. 벽에 막힌 상태나 공중에서는 플레이어 발소리가 나오지 않는다. 몬스터 포효는 `Attack` 상태로 들어가는 순간 한 번만 재생한다.

배경음악 파일과 연결 코드는 준비되어 있지만 현재는 주석 처리되어 재생하지 않는다. 플레이어 사망이나 탈출 성공 시에는 각각 승패음을 한 번 재생한다.

외부 녹음의 라이선스와 가공 내용은 `Assets/Audio/AUDIO_LICENSES.md`에 기록했다. 플레이어 비명은 CC-BY 3.0이므로 `SFX by Dan Knoflicek` 표기가 필요하다. 심장음은 실제 사람의 심장 녹음을 게임용 반복 구간으로 가공했고, 몬스터 발소리와 심장음은 작은 스피커에서도 들리도록 원본 파형의 음량도 높였다.

다음 큰 작업은 빛을 받으면 움직이고 어두우면 멈추는 두 번째 몬스터를 추가하는 것이다.
